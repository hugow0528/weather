...
def get_weather_data():
    # 獲取天文台數據
    curr_req = requests.get("https://data.weather.gov.hk/weatherAPI/opendata/weather.php?dataType=rhrread&lang=tc", timeout=15)
    fore_req = requests.get("https://data.weather.gov.hk/weatherAPI/opendata/weather.php?dataType=fnd&lang=tc", timeout=15)
    curr = curr_req.json()
    fore = fore_req.json()
    
    w = {
        "ts": datetime.datetime.now(pytz.timezone('Asia/Hong_Kong')).strftime('%m月%d日 %H:%M (%A)'),
        "temp": None, "hum": None, "warn": "", "future": ""
    }
...
def ask_ai(w):
    log("AI 諗緊建議...")
    prompt = f"""
    你係我嘅好朋友。請用廣東話口語簡單建議我今日點著衫。
    
    【今日數據】
    氣溫：{w['temp']}度, 濕度：{w['hum']}%, 警告：{w['warn']}
    未來天氣：{w['future']}
    
    【規則】
    1. 唔好講廢話，唔好講「唔使著咩」，直接講「要著咩」。
    2. 【返學】清單：恤衫(夏季短袖/冬季長袖), 長褲, 毛衣(背心/長袖), PE外套(風褸), 校褸, 底衫(長/短/保暖/背心), 打底褲(厚/薄/加薄)。
    3. 必須包含一件「底衫」(例如: 短袖底衫/背心底衫)。
    4. 20度以上請選「夏季短袖恤衫」。
    5. 只有15度以下才提「校褸」。
    6. 根據雨量(PSR)，喺「碎碎念」提醒要唔要帶遮。
    ...
    for model in ["gemini-fast", "openai-fast"]:
        try:
            res = requests.post(
                ...
                json={
                    "model": model,
                    "messages": [{"role": "system", "content": "你係一個說話直接、果斷嘅朋友。"}, {"role": "user", "content": prompt}],
                    "temperature": 0.5
                }, timeout=25
            )
            return res.json()['choices'][0]['message']['content'].strip()
        except: continue
    return "連唔到 AI，今日自己執生。"
